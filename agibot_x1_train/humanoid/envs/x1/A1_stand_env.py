...
class A1DHStandEnv(LeggedRobot):          
   # 在初始化函数中添加状态追踪变量
...
######################################################################################################
        self.last_is_lay = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        self.last_is_half_up = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        self.last_is_stand = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)        
###################################################################################################
...
#######################################################################################
    ...
    def _reward_lay_to_half_up(self):
       ## """奖励从平躺到半起身的转换"""
        base_z = self.root_states[:, 2]  # 基座高度
        base_rot = self.root_states[:, 3:7]  # 姿态四元数
        target_rot = torch.tensor([0, 0, 0, 1], device=self.device)  # 目标直立姿态
        rot_error = torch.norm(base_rot - target_rot, dim=1)  # 姿态误差

        # 定义状态
        is_lay = (base_z < self.cfg.rewards.lay_height_threshold) & (rot_error > 1.0)  # 仰卧
        is_half_up = (base_z >= self.cfg.rewards.lay_height_threshold) & (base_z < self.cfg.rewards.half_up_threshold) & (rot_error >= 0.5) & (rot_error < 1.0)  # 半起身

        # 检测从平躺到半起身的转换
        lay_to_half = is_half_up & ~self.last_is_half_up & self.last_is_lay

        # 更新状态缓存
        self.last_is_lay = is_lay
        self.last_is_half_up = is_half_up

        return lay_to_half * self.cfg.rewards.scales.lay_to_half_up

#################################################################################
    def _reward_base_ang_vel(self):
        #"""惩罚机身旋转角速度，减少站立时的晃动"""
        # 计算机身旋转角速度的模长（x、y、z轴合速度）
        ang_vel_magnitude = torch.norm(self.base_ang_vel, dim=1)
        return ang_vel_magnitude

    def _reward_half_to_stand(self):
       ## """奖励从半起身到站立的转换"""
        base_z = self.root_states[:, 2]  # 基座高度
        base_rot = self.root_states[:, 3:7]  # 姿态四元数
        target_rot = torch.tensor([0, 0, 0, 1], device=self.device)  # 目标直立姿态
        ...
        # 与配置文件的阶段阈值对齐
        is_half_up = (base_z >= self.cfg.rewards.lay_height_threshold) & (base_z < self.cfg.rewards.half_up_threshold) & (rot_error >= 0.5) & (rot_error < 1.0)  # 半起身
        is_stand = (base_z >= self.cfg.rewards.stand_threshold) & (rot_error < 0.5)  # 站立

        # 检测首次转换
        half_to_stand = is_stand & ~self.last_is_stand & self.last_is_half_up  

        # 更新状态缓存
        self.last_is_half_up = is_half_up
        self.last_is_stand = is_stand

        return half_to_stand * self.cfg.rewards.scales.half_up_to_stand
...
#######################################################################################
    def _reward_ref_joint_pos(self):
        ...
        return r
##############################################################################
    def _reward_half_up_to_stand(self):
    ##"""
    ##奖励机器人从半起身状态到站立状态的过程（当基座高度超过0.5m时）
    ##"""
    # 基座高度（假设从self.base_pos获取，具体需根据实际代码调整）
        base_height = self.base_pos[:, 2]
    # 当高度超过0.5m时给予奖励，否则为0
        return torch.where(base_height > 0.5, torch.tensor(1.0, device=self.device), torch.tensor(0.0, device=self.device))
##############################################################################    
    ...
    def _reward_low_height_move(self):
    # """
    #惩罚低姿态（未站立）时的移动行为：
    # - 低姿态定义：基座高度 < 0.5m（未达到站立标准）
    #- 移动定义：基座线速度 > 0.1m/s（有明显位移）
    # 仅在“低姿态且移动”时触发惩罚，匹配配置文件中的 `low_height_move = -5.0` 权重
    #"""
    ## 1. 获取当前机器人关键状态
        base_z = self.root_states[:, 2]  # 基座高度（z轴位置，单位：m）
        base_lin_vel = self.root_states[:, 7:10]  # 基座线速度（x/y/z方向，单位：m/s）
        vel_magnitude = torch.norm(base_lin_vel, dim=1)  # 计算移动速度的大小（合速度）
    
    # 2. 定义“低姿态”和“移动”的判断条件
        is_low_height = base_z < 0.5  # 高度 < 0.5m 视为未站立（低姿态）
        is_moving = vel_magnitude > 0.1  # 速度 > 0.1m/s 视为有移动
    
    # 3. 仅在“低姿态且移动”时触发惩罚（负奖励）
    # 用布尔掩码筛选需要惩罚的环境，再乘以配置中的权重
        punishment_mask = is_low_height & is_moving
        return punishment_mask * self.cfg.rewards.scales.low_height_move
    ...
    def _reward_stand_still(self):
        # 1. 判断是否为“站立指令”且已达到站立高度
        stand_command = (torch.norm(self.commands[:, :3], dim=1) <= self.cfg.commands.stand_com_threshold)
        is_stand_height = self.root_states[:, 2] >= self.cfg.rewards.stand_threshold  # 高度达标
        is_qualified = stand_command & is_stand_height  # 同时满足“指令静止”和“高度达标”
        
        # 2. 高度奖励（接近目标高度0.61m）
        base_height = self.root_states[:, 2]
        height_reward = torch.exp(-5 * torch.square(base_height - self.cfg.rewards.base_height_target))
        
        # 3. 姿态奖励（直立，无倾斜）
        base_rot = self.root_states[:, 3:7]
        rot_error = torch.norm(base_rot - torch.tensor([0,0,0,1], device=self.device), dim=1)
        rot_reward = torch.exp(-10 * rot_error)
        
        # 4. 关节位置奖励（保持稳定姿态）
        joint_pos_reward = torch.exp(-torch.sum(torch.square(self.dof_pos - self.default_dof_pos), dim=1))
        
        # 5. 综合奖励：仅对合格状态生效
        r = height_reward * 0.4 + rot_reward * 0.3 + joint_pos_reward * 0.3
        r = torch.where(is_qualified, r, torch.zeros_like(r))
...
#################################################################0928       
    ...
    def compute_reward(self):
        ...
        # 保留原有阶段过渡奖励
        rewards += self._reward_lay_to_half_up()  # 平躺到半起身
        rewards += self._reward_half_to_stand()   # 半起身到站立
        rewards += self._reward_stand_still() * self.cfg.rewards.scales.stand_still  # 稳定站立
    
        # 新增：基础姿态与高度奖励（确保初始有信号）
        rewards += self._reward_orientation() * self.cfg.rewards.scales.orientation  # 直立姿态奖励
        rewards += self._reward_base_height() * self.cfg.rewards.scales.base_height  # 目标高度奖励
        rewards += self._reward_default_joint_pos() * self.cfg.rewards.scales.default_joint_pos  # 关节位置惩罚
    
        # 新增：动作平滑性惩罚（避免剧烈动作）
        rewards -= self._reward_action_smoothness() * self.cfg.rewards.scales.action_smoothness
